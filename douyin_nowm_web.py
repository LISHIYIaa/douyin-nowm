...
def xhs_extract_note_info(initial_state: dict, note_id: str) -> dict:
    ...
    if note_type == "video":
        ...
        if image_list:
            base["cover_url"] = image_list[0].get("urlDefault", "") or image_list[0].get("url", "")
        else:
            base["cover_url"] = ""
    else:
        # 图文笔记：提取所有图片
...
def process_link(share_url: str, quality: str = "default") -> dict:
    ...
    if platform == "douyin":
        return process_douyin(share_url, quality)
    elif platform == "xhs":
        return process_xhs(share_url)
        ...
        <div class="result-card">
          <div class="result-body">
            ...
            <div class="download-bar">
              ...
              <a class="btn-download" href="/api/download_zip?note_id=${data.video_id}&author=${encodeURIComponent(data.author)}&pub_date=${encodeURIComponent(data.pub_date||'')}" target="_blank">
                ⬇ 打包下载
              </a>
            </div>
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/" or parsed.path == "/index.html":
            ...
            if not note_id:
                ...
                return
            self._proxy_download_zip(note_id, author, pub_date)
    ...
    def _proxy_download_zip(self, note_id, author, pub_date):
        """重新获取小红书笔记页面，打包所有图片为 ZIP"""
        try:
            # 通过 note_id 构造页面 URL 并重新获取
